...
public class BulletConfig implements Serializable {
    ...
    private String getPath(){
        if (!this.shootSound.startsWith("file://")) {
            try {
                Class ma = Class.forName("com.orangomango.projectile.MainApplication");
                if (this.shootSound.startsWith("GUN_SOUNDS")) {
                    Field field = ma.getField("GUN_SOUNDS");
                    return ((Media[]) field.get(null))[Integer.parseInt(this.shootSound.split("\\[")[1].substring(0, this.shootSound.split("\\[")[1].length() - 1))].getSource();
                } else {
                    Field field = ma.getField(this.shootSound);
                    return ((Media) field.get(null)).getSource();
                }
            } catch (Exception ex) {
                ex.printStackTrace();
                return null;
